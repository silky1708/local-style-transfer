...
def load_seg_mask(seg_path, img_height=None, img_width=None):
    seg_mask = Image.open(seg_path)
    if img_width is not None:
        seg_mask = seg_mask.resize((img_width, img_height))  # change image size to (3, img_size, img_size)
    
    transform = transforms.Compose([
                    transforms.ToTensor(),
                ])   

    seg_mask = transform(seg_mask)[:3, :, :].unsqueeze(0)  # 1x3x512x512
    return seg_mask


def load_seg_mask2(seg_mask, img_height=None, img_width=None):
#     seg_mask = Image.open(seg_path)
    if img_width is not None:
        seg_mask = seg_mask.resize((img_width, img_height))  # change image size to (3, img_size, img_size)
